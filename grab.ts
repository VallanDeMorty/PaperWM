...
const prefs = Extension.imports.settings.prefs;
const Utils = Extension.imports.utils;
const Tweener = Utils.tweener;
...
function createDnDZones(space) {
  ...
  for (let i = 0; i < space.length; i++) {
    ...
    zones.push(
      mkTileZone(x, y + height + prefs.window_gap, width, [i, col.length])
    );
  }

  zones.push(
    mkColumnZone(space.cloneContainer.width + prefs.window_gap, [space.length])
  );
  if (space.length === 0) {
...
const MoveGrab = class MoveGrab {
  ...
  selectDndZone(initial = false) {
    ...
    if (newDndTarget !== this.dndTarget) {
      ...
      if (newDndTarget) this.activateDndTarget(newDndTarget, initial);
    }
  }
  ...
  positionChanged(metaWindow) {
    ...
    if (this.dnd) {
      ...
      // Move the window and scroll the space
      let space = this.initialSpace;
      let clone = metaWindow.clone;
      let frame = metaWindow.get_frame_rect();
    ...
    if (this.dnd) {
      ...
      clone.set_position(clone.targetX, clone.targetY);
  ...
  activateDndTarget(zone, first) {
    ...
    if (first) {
      ...
      zone.actor.set_position(
        ...space.globalToScroll(...actor.get_transformed_position())
      );
...
const ResizeGrab = class ResizeGrab {
  constructor(metaWindow, type) {
    this.window = metaWindow;
    this.signals = new Utils.Signals();
